% This is K-RVEA, Chenyan Gu changes it to the AFF.
function [off,Empty_ref_old] = evolve_K_RVEA_1(model_ex,model_nex,Archive,Boundary,Empty_ref_old,id_ex,id_nex,FE_ex,FE_max)
no_var = size(Boundary,2);
Population = Archive(:,1:no_var);
FunctionValue = Archive(:,no_var+1:end);

maxFE = 2000;
Coding = 'Real';

Vs = ref_vectors(2); % Generation of reference vectors
V = Vs;

N = size(V,1);
M = size(FunctionValue,2);
Fix_V = Vs;
up_var = 3;%M
alpha = 0.5; 
cosineVV = V*V';
scosineVV = sort(cosineVV, 2, 'descend');
acosVV = acos(scosineVV(:,2));
refV = (acosVV);

FE = 0;
w=0;
ERR = zeros(size(Population,1),1);
% metagen = round(maxFE/N) + 200;
while FE <maxFE
%% SBX and polynomial mutation    
        MatingPool= F_mating1(Population,N);
        Offspring = P_generator(MatingPool,Boundary,Coding,N); 
        
        ER = zeros(size(Offspring,1),M);
        
        Fitness = zeros(size(Offspring,1),M); 

        for i = 1:size(Offspring,1)
            x = Offspring(i,:);
            [Fitness(i,id_ex),~,ER(i,id_ex)] = predictor(x,model_ex);
            [Fitness(i,id_nex),~,ER(i,id_nex)] = predictor(x,model_nex);
        end
%         [Fitness(:,id_ex),ER(:,id_ex)] = predict(model_ex,Offspring);
%         
%         [Fitness(:,id_nex),ER(:,id_nex)] = predict(model_nex,Offspring);
        
        FE = FE + size(Fitness,1);
        ER = mean(ER,2);

        Population = [Population; Offspring];       
        FunctionValue = [FunctionValue; Fitness];
        ERR = [ERR;ER];
        if M==2%%%%%Aff not theta here.
            theta =  (FE/(maxFE))^alpha;
        else
             theta =  M*(w/(metagen))^alpha;%%%%%%%%%%metagen
        end
        Selection= F_select(FunctionValue,V, theta, refV);
        Population = Population(Selection,:);
        FunctionValue = FunctionValue(Selection,:);
        ERR = ERR(Selection,:);
        
        if(mod(FE, ceil(maxFE*0.3)) == 0 && FE > 0)
        
            Zmin = min(FunctionValue,[],1);	
            Zmax = max(FunctionValue,[],1);	

            V = Vs;

            V = V.*repmat((Zmax - Zmin)*1.0,N,1);
            tV = V;


            for i = 1:N
                V(i,:) = V(i,:)./norm(V(i,:));
            end

            cosineVV = V*V';
            [scosineVV, neighbor] = sort(cosineVV, 2, 'descend');
            acosVV = acos(scosineVV(:,2));

            refV = (acosVV);

        end
        w = w+1;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

a=-0.5*cos(FE_ex*pi/FE_max)+0.5;
b=1-a;
[MERR,~]=max(ERR,[],1);
[MObj,~]=max(FunctionValue,[],1);
Nondominate = P_sort(FunctionValue,'first')==1;
Best_pop=Population(Nondominate,:);
fit=FunctionValue./repmat(MObj,size(FunctionValue,1),1)*b+ERR./repmat(MERR,size(FunctionValue,1),1)*a;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
info_update = struct('c',{fit,V,theta,Fix_V,Empty_ref_old,refV,up_var,N,Population,ERR,Boundary});
[off,Empty_ref_old] = update_metamodel(info_update);       
off = unique(off,'rows');
current_pop = Population;


P_archive = Archive(:,1:no_var);

Lia = ismember(off,P_archive,'rows');
r_unique = find(Lia(:,1)==0);
off = off(r_unique,:);

tt = 1;
while (isempty(off))
    rt = randperm(size(current_pop,1));
    off = current_pop (rt(1:1),:);
    Lia = ismember(off,P_archive,'rows');
    r_unique = find(Lia(:,1)==0);
    off = off(r_unique,:);
    tt = tt+1;
    if tt>1000
        break;
    end
end 
end
                
              

